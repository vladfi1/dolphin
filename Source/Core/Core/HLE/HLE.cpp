...
constexpr std::array<Hook, 24> os_patches{{
    ...
    {"HSD_RandiTrace",               HLE_Misc::HLE_HSD_RandiTrace,          HookType::Start,   HookFlag::Fixed},
...
void PatchFixedFunctions(Core::System& system)
{
  ...
  // Needle post-hit RNG forensics: hook HSD_Randi (GALE01 0x80380580) by address when
  // MSL_RNG_TRACE is set. Installed here at boot (before the function is JIT-compiled), so the JIT
  // picks up the HookType::Start hook with no unsafe mid-execution cache clear. Inert unless
  // MSL_RNG_TRACE is set.
  if (std::getenv("MSL_RNG_TRACE") != nullptr)
  {
    Patch(system, 0x80380580, "HSD_RandiTrace");
  }
}
